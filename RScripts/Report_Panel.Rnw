...
\ifx{\Sexpr{length(which(mutation_analysis_result_mutect2$som_mut_tab$is_oncogene == 1 | mutation_analysis_result_mutect2$som_mut_tab$is_tumorsuppressor == 1 | mutation_analysis_result_mutect2$som_mut_tab$InterVar_automated %in% c("Pathogenic", "Likely pathogenic") | mutation_analysis_result_mutect2$som_mut_tab$CLNSIG %in% c("Pathogenic", "Likely_pathogenic", "Conflicting_interpretations_of_pathogenicity")))}}0
{\textbf{Keine Mutationen in Cancergenen und keine Mutationen mit hoher ACMG-Klassifikation identifiziert.}}
\else
{
<<highlight, echo=FALSE, eval=TRUE, results="tex", warning=FALSE>>=
tmp <- highlight_table$highlight
if(dim(tmp) != 0) {
  cap <- "\\footnotesize{Mutationen mit ACMG Klassifizierungen: Warscheinlich Pathogen (4), Pathogen (5), Conflicting Interpretations of Pathogenicity (4*). Revel: \\textbf{D}(eleterious), \\textbf{N}(eutral). Hotspots sind \\textbf{fett} dargestellt.}"
...
In der Analyse werden nur seltene Mutationen mit einer \ac{VAF} über \Sexpr{vaf}\% betrachtet. Gezeigt werden Mutationen vom Typ \acp{SNV}, \acp{DNV} und \acp{InDels}. Letztere sind Mutationen, die in der Keimbahn heterozygot vorliegen, im Tumor allerdings homozygot.\\
...
Die Lage der Mutationen im Genom wird, aufgeteilt nach Mutationstyp (SNV und InDels), im Circosplot dargestellt (Abb \ref{cir_plot}). Die gelben Bereiche sind die Regionen, die das Panel abdeckt. Wenn Fusionen in der Probe identifiziert wurden, werden diese im inneren Kreis abgebildet.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.53\textwidth]{\Sexpr{outfile_circos_mutect2}}
\caption{Circos Plot mit allen Mutationen \"uber VAF \Sexpr{vaf}.}
\label{cir_plot}
\end{figure}
\clearpage
...
In der folgenden Tabellen werden Gensymbol und der entsprechende Aminos\"aurenaustausch gelistet.
Die Funktion der Mutationen werden durch Abk\"urzungen beschrieben, welche im Ahang aufgef\"uhrt sind.
...
Am Ende wird die Zuordnung des Gens als Cancergene nach der Klassifikation von OncoKB dargestellt. \textbf{Mutationen in einem \ac{HS}} sind fett markiert. Die folgende Tabelle \ref{som_mut_cg} enth\"alt alle \acp{SNV}, \acp{DNV} und \acp{InDel} in Cancergenen f\"ur diese Probe.

Bei den \textbf{Gensymbolen} ist ein Link zu \textbf{\textit{Genome Nexus}} hinterlegt. Dort wird ein Lollipop-Plot zur Lage der Mutation im Gen angezeigt sowie weitere Informationen. Beim \textbf{Aminosäurenaustausch} ist ein Link zum Eintrag der Mutation in der \textbf{\ac{VICC}} verkn\"upft. Bei der \textbf{Funktion} ist ein Link zu \textbf{\ac{VarSome}} hinterlegt. Dort sind auch \acp{InDel} und \acp{DNV} nach ACMG klassifiziert. Diese Links funktionieren allerdings aufgrund eines Sonderzeichens in der URL nicht bei Verwendung des Dokuments mit der Vorschau von Apple.
...
\subsection{CNVs}
Wie bei den Mutationen werden auch bei den CNVs betroffene Cancergenen (Onkogene und Tumorsuppressoren) genauer betrachtet und in folgenden Tabellen dargestellt. Dabei werden für Gene in Tabelle \ref{cnv_og} Losse und Gains von Kopien dargestellt.

<<eval=TRUE, results="tex", echo=FALSE, warning = FALSE>>=
kable(cnvs, caption = "Identifizierte \\acp{CNV}\\label{cnv_og}", booktabs = T, row.names = F, digits = 2) %>% kable_styling(latex_options = c("striped", "HOLD_position"), font_size = 9, full_width = F) %>% column_spec(c(1), width = "5em") %>% column_spec(c(2:3), width = "1em") %>% row_spec(0, angle = 60)
@

\clearpage
\section{Wichtige Signalwege}\label{imp_pathways_section}
Es werden fünf wichtige Signalwege betrachtet: \ac{RAF-MEK-ERK}, \ac{PI3K-AKT-mTOR}, \ac{DamResp}, \ac{CellCycle} und \ac{TyrKin}. Es wird untersucht, welche Gene in diesen Signalwege eine Mutation tragen. Diese Mutationen sind analog zu Kapitel \ref{som_mut_section} in Tabelle \ref{imp_ptws} dargestellt. Wenn in einem Signalweg keine Mutation identifiziert wurde, taucht er in der Tabelle nicht auf.\\
...
if (is.null(tmp)) {
  ...
  kable(tmp, format = "latex", caption = "Identifizierte somatische Mutationen in wichtigen Signalwegen\\label{imp_ptws}", booktabs = T, longtable = T, row.names = F, escape = F) %>% kable_styling(latex_options = c( "HOLD_position", "repeat_header"), font_size = 9) %>% column_spec(c(3, 4, 9), width = "5em") %>% column_spec(c(1:2, 5:6, 7), width = "3em") %>% column_spec(c(8), width = "1em") %>% column_spec(1, bold = T) %>% row_spec(0, angle = 60)
}
@
\clearpage
...
if (is.null(tmp)) {
  ...
  kable(tmp, format = "latex", caption = " Identifizierte CNVs in wichtigen Signalwegen\\label{imp_ptws_cnv}", booktabs = T, longtable = T, row.names = F) %>% kable_styling(latex_options = c("HOLD_position", "repeat_header"), font_size = 9, full_width = F) %>% column_spec(1, width = "15em") %>% column_spec(c(2:3), width = "5em") %>% column_spec(4, width = "30em") %>% column_spec(1, bold = T) %>% row_spec(0, angle = 60)
}
@ 
...
Untersucht werden die Mutationssignaturen basierend auf den COSMIC Mutation Signatures (V1) für alle Mutationen in der Probe. In der folgenden Tabelle \ref{mut_sig} werden nur Signaturen gelistet, die einen Anteil größer als 1\% haben. Die Signatur AC3 wird als \textit{BRCAness} bezeichnet.\\
...
tmp <- tmp[, c(1:3)]
tmp[,3] <- tmp[,3]*100
colnames(tmp) <- c("Signatur", "Beschreibung", "Anteil in %")
...
if (is.null(tmp)) {
  ...
  kable(tmp, format = "latex", caption = "Mutationen in TOPART-G1-Genen\\label{top_art}.", booktabs = T, longtable = T, row.names = F, escape = F) %>% kable_styling(latex_options = c( "HOLD_position", "repeat_header"), font_size = 9) %>% column_spec(c(1:3, 4, 9), width = "5em") %>% column_spec(c(2:3, 8), width = "3em") %>% column_spec(c(7), width = "2em") %>% column_spec(1, bold = T) %>% row_spec(0, angle = 60)
  }
@
\clearpage

\section{Fusionen}
\textbf{CAVE: Die Analysen der RNA-Fusionen sind vorl\"aufig. Das Ergebnis ist auf die Fusionsgene von Illumina beschr\"ankt.}
In folgender Tabelle \ref{fs_ov} werden \"ubersichtlich alle \textit{in-frame} und \textit{out-of-frame} Fusionen mit vier wichtigen Eigenschaften (bekannt, reziprok, readthrough und Cosmic-Annotation dieser Genkombination) dargestellt. Dabei muss immer nur eines der beiden Fusionsgene in der vorgegebenen Genliste stehen. Je h\"oher die Fusion in der Tabelle steht, desto \"ofter wurde die Fusion in der Probe detektiert.

Die Reads, welche die Fusion unterst\"utzen sind in Tabelle \ref{fs_sup} als \textit{Split Reads}, welche den Bruchpunkt direkt beinhaltend, und als \textit{Supporting} unterteilt.

In Tabelle \ref{fs_bp} werden zu den Fusionen alle m\"oglichen Bruchpunkte in genomischen Koordinaten bez\"uglich \textit{hg19} beschrieben.
Tabelle \ref{fs_plot} zeigt f\"ur Fusionen, bei denen jeweils beide Bruchpunkte in einer codierenden Region liegen, einen Plot des Fusionsprodukten mit den erhaltenen Proteindomainen. Die Tabelle \ref{fs_sq} gibt die Sequenz um den Bruchpunkt an. Dabei wird aus \"Ubersichtlichkeitsgr\"unden die Sequenz für jedes, an der Fusion beteiligte, Gen getrennt angegeben.
...
<<echo=FALSE, results='tex', eval=TRUE>>=
tmp <- fusions$Table[, c(1,3,8,7)]
if (!is.null(tmp)) {
  id <- which(tmp[,3] >= 3)
  if (length(id) > 0) {
    kable(tmp, format = "latex", caption = "Unterst\"uzende Reads der identifizierten Fusionen \\label{fs_sup}", booktabs = T, longtable = T, digits = 3, row.names = F) %>% kable_styling(latex_options = c("striped", "HOLD_position", "repeat_header"), font_size = 10) %>% row_spec(id, bold = TRUE)
  } else {
    kable(tmp, format = "latex", caption = "Unterst\"uzende Reads der identifizierten Fusionen \\label{fs_sup}", booktabs = T, longtable = T, digits = 3, row.names = F) %>% kable_styling(latex_options = c("striped", "HOLD_position", "repeat_header"), font_size = 10)
  }
}
@
 \clearpage
...
if(!is.null(tmp)){
  ...
  if(length(id) > 0) {
    ...
    tmp$plots <- paste0("\\includegraphics[height=4cm]{", tmp$file, "}")
  ...
  kable(tmp, format = "latex", escape = FALSE, caption = "Plot identifizierten Fusionen. Der senkrechte Strich markiert den Bruchpunkt. \\label{fs_plot}", booktabs = T, longtable = T, digits = 3, row.names = F) %>% kable_styling(latex_options = c("striped", "HOLD_position", "repeat_header"), font_size = 10) %>% column_spec(1,extra_css = "vertical-align:middle;")
  }
@
...
\caption{Darstellung der Sequenzierung als Output aus dem Sequenzierer, (a), und nach der \ac{BQSR}, (b)}
\label{base_qual}
\end{figure}
\clearpage
...
<<eval=TRUE, results="tex", echo=FALSE, warning=FALSE>>=
df <- data.frame(
  Kriterium = c('Librarygr\"o\"se', "Mean Coverage", "Basen > 50 Reads", "Basen > 150 Reads", "Exonische Basen > 50 Reads", "Exonische Basen > 150 Reads", 'Insertgr\"o\"se (Standardabweichung)', "QC-Anteil", 'Mean Qualit\"at pro Base'),
  Tumor = c(
  ...
  Mindestwert = c("50 Mio Reads", "1000 X", "90%", "90%", "90%", "90%", "100bp", "40-60%", 28)
)
...
Die Mean-Coverage und die Gesamtcoverage der Proben ist ein weiteres Qualitätsmerkmal. Sind die Werte zu niedrig, sind viele Stellen im Genom unzureichend abgedeckt. Abbildung \ref{cov_plot} zeigt die kummulative Verteilung, durch die der Anteil an targeted Basen, die von einer bestimmten Anzahl an Reads abgedeckt sind, beschrieben wird.
...
\begin{acronym}[PI3K-AKT-mTOR]
  ...
  \acro{nfsDel}[nfsDel]{nonframeshift deletion / in-frame deletion}
  \acro{nfsSub}[nfsSub]{nonframeshift substitution / missense mutation}
  \acro{fsDel}[fsDel]{frameshift deletion}
  \acro{nfsIns}[nfsIns]{nonframeshift inserstion / in-frame instertion}
  \acro{nfIns}[nfIns]{frameshift insterstion}
