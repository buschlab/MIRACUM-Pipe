...
make_cnv_graph <- function(ratio_file, ploidity = '2', outfile_plot) {
  ...
  gtrellis_layout(n_track = 3, nrow = 5, byrow = F, equal_width = FALSE,
                  ...
                  track_height = unit.c(2 * grobHeight(
                    textGrob("chr1")),
                    unit(1, "null"),
  ...
  tt <- which(
    ratio_new$Ratio == maxLevelToPlot & ratio_new$CopyNumber > ploidy
  )
  tmp <- ratio_new[tt,]
  add_track(tmp, track = 2, panel_fun = function(tmp) {
    x <- tmp$Start
  ...
  add_track(
    track = 1, tmp, panel_fun = function(gr) {
      grid.rect(
        x = gr$start,
        y = unit(0.2, "npc"),
        width = (gr$end - gr$start),
        height = unit(0.8, "npc"),
        hjust = 0, 
        just = 0,
        default.units = "native",
        gp = gpar(
          fill = col_fun(gr$CopyNumber),
          col = NA
        )
      )
    }
...
del_dup_query <- function(da.fr, server) {
  ...
  out <- mclapply(1:length(ucscchroms), function(i) {
    ...
    stop <- stop_list[i]
    tmp <- paste0(
      "Select name2 FROM refGene WHERE chrom='",
      chrom,
      "'AND ((cdsStart BETWEEN'",
      start,
      "'AND'",
      stop,
      "') OR (cdsEnd BETWEEN'",
...
offline_query <- function(location, db) {
  ...
  out <- genes(
    db,
    filter = ~ seq_name == chr &
    ((tx_start >= start & tx_start <= end) |
    (tx_end >= start & tx_end <= end))
  )
...
# Try annotation first with SQL server if this fails try with biomaRt, if no internet connection available use offline
  ...
  if (cnv_region_annotation == "offline") {
    print("Offline CNV Region annotation mode.")
    for(i in 1:nrow(x)) {
      cat("Processing CNV#", i, "\n")
  ...
  onco <- x1[x1$Oncogene != ".", c(
    "chr",
    "copy.number",
    "status",
    "Oncogene"
  )]
  tumorSupp <- x1[x1$TumorSuppressor != ".", c(
    "chr",
    "copy.number",
    "status",
    "TumorSuppressor"
  ...
  return(
    list(
      CNVsAnnotated = x1,
      CNVOncogenes = onco,
      CNVTumorSuppressors = tumorSupp
    )
  )
}


cnv_processing <- function(
  cnv_file,
  targets,
  db,
  path_data,
...
# LOSS
  ...
  loss_go <- get_terms(
    dataset = db, 
    mut.entrez = prep$de_genes,
    t2.entrez = prep$universe,
    outfile = NULL
  )
  ########
  # GAIN #
...
cnvs2cbioportal <- function(cnvs, id, outfile_cbioportal, gender, ampl_genes = NULL, ploidy) {
  ...
  if(!is.null(ampl_genes)) {
    ampl_genes.entrez <- unlist(
      lapply(
        mget(
          as.character(ampl_genes),
          org.Hs.egSYMBOL2EG,
          ifnotfound = NA
        ), function(x) x[1]
      )
    )
    cnvs.complete <- data.frame(
      Hugo_Symbol = ampl_genes,
      Entrez_Gene_Id = ampl_genes.entrez,
      Sample_ID = 2
    )
    cnvs.complete[na.omit(
      match(
        cnvs.out$Hugo_Symbol,
        cnvs.complete$Hugo_Symbol
      )
    ), "Sample_ID"] <- cnvs.out$Sample_ID
...
get_type <- function(
  ...
  if(sureselect_type != "TSO500") {
    ...
    gene_loci_tsg <- genes_edb[which(
      genes_edb$symbol %in% genes_tsg
    ), ]
    gene_loci_tsg <- gene_loci_tsg[which(
      gene_loci_tsg$gene_biotype == "protein_coding"
    ), ]
    gene_loci_tsg <- gene_loci_tsg[which(
      seqnames(gene_loci_tsg) %in% c(1:22, "X")
    )]

    gene_loci_onc <- genes_edb[which(
      genes_edb$symbol %in% genes_onc
    ), ]
    gene_loci_onc <- gene_loci_onc[which(
      gene_loci_onc$gene_biotype == "protein_coding"
    ), ]
    gene_loci_onc <- gene_loci_onc[which(
      seqnames(gene_loci_onc) %in% c(1:22, "X")
    )]
    ...
    for (i in 1:dim(gene_loci)[1]) {
      if(length(get_cn(gene_loci[i, ])) > 0) {
        ...
        gene_loci$cn[i] <- paste0(
          Oncogenes$CopyNumber[grep(gene_loci$gene_name[i],
          x = Oncogenes$Gene)],
          sep = "", collapse = ", "
        )
      }
...
freec2seg <- function(cnvs_file, cpn_file, id, outfile_seg) {
  ...
  for (i in 1:nrow(bam)) {
    ...
    bam[i,]$num.mark <- sum(
      cpn[
        cpn$V1 == entry$chrom &
        (cpn$V2 >= entry$loc.end & cpn$V2 <= entry$loc.end |
        cpn$V3 >= entry$loc.start & cpn$V3 <= entry$loc.end
        ),
      ]$V4
    )
  }
