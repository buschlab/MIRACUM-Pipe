  ...
  if(nrow(x) > 0) {
    for(i in 1:nrow(x)) {
      ...
      if(length(transcript) == 0) {
        transcript <- str_split(ref[i], ",", simplify = F)[[1]][1]
      }
...
txt2maf <- function(input, snv_vcf, protocol, Center = center, refBuild = 'GRCh37', idCol = NULL, id = NULL, sep = "\t", Mutation_Status = c("T", "N","LOH")[1]){
