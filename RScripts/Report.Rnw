...
if (protocol == "SomaticGermline"){
  if (length(helper) == 1){
    tmp1 <- data.frame(Eigenschaften = c("Capture Kit", "Abgedeckte Region", paste0("Mutationslast (exonisch, VAF > ", vaf, "%)"), paste0("Anzahl somatischer Mutationen (VAF > ", vaf, "%)"), "BRCAness", "Mikrosatelliten Status", "Anzahl CN- Regionen", paste0("Anzahl seltener Keimbahnmutationen (VAF > ", vaf, "%)")), Wert = c(as.character(sureselect_type), paste(covered_region, "Mb", sep = ""), paste0(round(x = filt_result_td$tmb, digits = 2), "/Mb"), as.character(round(x = sum(as.numeric(mutation_analysis_result$mut_tab[, 2])), digits = 0)), paste0(round(x = mut_sig_ana$output$Summary[helper, 3], digits = 2), " %"), as.character(paste0(as.character(filt_result_td$msi$result$MSI_status[1]), " (", round(x = filt_result_td$msi$scores$ratio*100, digits = 2), "%)")[1]), paste0(round(x = dim(cnv_analysis_results$cnvs_annotated$CNVsAnnotated)[1], digits = 0), " Regionen"), length(which(as.numeric(gsub(pattern = "%", replacement = "", x = filt_result_gd$table$Variant_Allele_Frequency[-which(is.na(filt_result_gd$table$ExonicFunc.refGene))])) > 10))))
    } else {
      tmp1 <- data.frame(Eigenschaften = c("Capture Kit", "Abgedeckte Region", paste0("Mutationslast (exonisch, VAF > ", vaf, "%)"), paste0("Anzahl somatischer Mutationen (VAF > ", vaf, "%)"), "BRCAness", "Mikrosatelliten Status", "Anzahl CNV- Regionen", paste0("Anzahl seltener Keimbahnmutationen (VAF > ", vaf, "%)")), Wert = c(as.character(sureselect_type), paste(covered_region, "Mb", sep = ""), paste0(round(x = filt_result_td$tmb, digits = 2), "/Mb"), round(x = sum(as.numeric(mutation_analysis_result$mut_tab[, 2])), digits = 0), "< 1.0%", as.character(paste0(as.character(filt_result_td$msi$result$MSI_status[1]), " (", round(x = filt_result_td$msi$scores$ratio*100, digits = 2), "%)")[1]), paste0(round(x = dim(cnv_analysis_results$cnvs_annotated$CNVsAnnotated)[1], digits = 0), " Regionen"), length(which(as.numeric(gsub(pattern = "%", replacement = "", x = filt_result_gd$table$Variant_Allele_Frequency[-which(is.na(filt_result_gd$table$ExonicFunc.refGene))])) > 10))))
    }
} else {
    if (length(helper) == 1){
