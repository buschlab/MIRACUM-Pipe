...
tmp1 <- data.frame(Eigenschaften = c("Capture Kit", "Abgedeckte Region", paste0("Mutationslast (exonisch, VAF > ", vaf, "%)"), paste0("Anzahl Mutationen (VAF > ", vaf, "%)"), "Anzahl CNV- Regionen"), Wert = c(as.character(sureselect_type), paste(covered_region, "Mb", sep = ""), paste0(round(x = filt_result_td$tmb, digits = 2), "/Mb"), round(x = sum(as.numeric(mutation_analysis_result$mut_tab[, 3])), digits = 0), paste0(round(x = dim(cnv_analysis_results$cnvs_annotated$CNVsAnnotated)[1], digits = 0), " Regionen")))
...
\begin{figure}[h]
\centering
  \subfloat{\includegraphics[height = 0.45\textwidth]{\Sexpr{outfile_circos}}}
...
\clearpage
\section{Mutationssignaturen}
Untersucht werden die Mutationssignaturen basierend auf den COSMIC Mutation Signatures (V2) für alle somatischen Mutationen in der Probe. In der folgenden Tabelle \ref{mut_sig} werden nur Signaturen gelistet, die einen Anteil größer als 1\% haben. Die Signatur AC3 wird als \textit{BRCAness} bezeichnet.

<<mut_sig, echo=FALSE, eval=TRUE, results='tex'>>=
tmp <- mut_sig_analysis$CosmicValid_cutoffGen_LCDlist
tmp <- data.frame(tmp$out_sig_ind_df$sig, tmp$out_sig_ind_df$process)
rownames(tmp) <- tmp[,1]
tmp <- merge(tmp,mut_sig_analysis$CosmicValid_cutoffGen_LCDlist$norm_exposures,by="row.names",all.x=TRUE)[,-c(1)]
tmp[,3] <- tmp[,3]*100
...
\begin{table}[h]
  ...
  Anteil der Basen > 8 Reads & \Sexpr{round(stats$cover$perc[[1]][1], digits = 2) * 100} \% & >90\% \\
  Anteil der Basen > 40 Reads & \Sexpr{round(stats$cover$perc[[1]][2], digits = 2) * 100} \% & Sehr gut: >90\%, akzeptabel: >60\% \\
  Anteil der exonischen Basen > 8 Reads & \Sexpr{round(stats$cover_exons$perc[[1]][1], digits = 2) * 100} \% & >90\% \\
  Anteil der exonischen Basen > 40 Reads & \Sexpr{round(stats$cover_exons$perc[[1]][2], digits = 2) * 100} \% & Sehr gut: >90\%, akzeptabel: >60\% \\
  Insertgröße & \Sexpr{stats$avreads$tin} $\pm$  \Sexpr{stats$avreads$tin_sd}bp & >100 bp \\
  GC-Anteil & \Sexpr{stats$qc_check$gc_content[[1]]} \% & 40-60 \% \\
