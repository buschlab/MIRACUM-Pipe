...
<<eval=TRUE, results="tex", echo=FALSE>>=
kable(sq_tab, booktabs = T, row.names = F, caption = "\\footnotesize{Auff{\"a}lligkeiten in Qualit{\"a}tsmerkmalen der Proben}") %>% kable_styling(latex_options = c( "hold_position"), font_size = 9)
@
...
<<eval=TRUE, results="tex", echo=FALSE, warning=FALSE>>=
df <- data.frame(
  Kriterium = c('Librarygr\"o\"se', "Mean Coverage", "Basen > 8 Reads", "Basen > 40 Reads", "Exonische Basen > 8 Reads", "Exonische Basen > 40 Reads", 'Insertgr\"o\"se (Standardabweichung)', "QC-Anteil", 'Mean Qualit\"at pro Base'),
  Tumor = c(
  ...
  Mindestwert = c("100 Mio Reads", "80 X", "90%", "90%", "90%", "90%", "100bp", "40-60%", 28)
)
