...
${BIN_RSCRIPT} "${DIR_RSCRIPT}/Main.R" "${CFG_CASE}" "${PARAM_DIR_PATIENT}" "${CFG_FILE_GERMLINE_R1}" "${CFG_FILE_TUMOR_R1}" \
  ...
  "${CFG_REFERENCE_ACTIONABLEGENES}" "${CFG_REFERENCE_COVERED_EXONS}" "${CFG_UCSC_SERVER}"

# translate to tex
${BIN_RSCRIPT} --vanilla -e "load('${DIR_ANALYSES}/MTB.RData'); library(knitr); knit('${DIR_RSCRIPT}/Report_tumorOnly.Rnw');"
